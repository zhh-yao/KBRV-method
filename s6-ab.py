...
for i in range(len(number)):
    for j in range(len(sample)):
        for k in range(len(c)):
            ...
            df.to_csv('D:/csv_lnl/combination%d-%d-%d.csv'%(i,j,k), index = False, header = True)
...
for i in range(len(number)):
    for j in range(len(sample)):
        for k in range(len(c)):
            data = pd.read_csv('D:/csv_lnl/combination%d-%d-%d.csv'%(i,j,k))
            ...
            fpr5, tpr5, threshold5 = roc_curve(1-data['Label'], data['KBRV5'])
