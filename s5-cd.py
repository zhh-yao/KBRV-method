...
def dataGenerate_dep(num1_exon, num2_exon, S, co):
    ...
    if num1_exon >= num2_exon:
        ...
        isoform2_expression = isoform2_expression + co*np.cos((isoform1_expression[:,0:num2_exon]))
    else:
        isoform1_expression = isoform1_expression + co*(isoform2_expression[:,0:num1_exon])
        isoform2_expression = isoform2_expression + co*np.cos((isoform2_expression[:,0:num2_exon]))
...
def dataGenerate_ind(num1_exon, num2_exon, S):
    '''
    num1_exon: number of exon in gene 1;
    num2_exon: number of exon in gene 2;
    S: sample size;
...
for i in range(len(number)):
    for j in range(len(sample)):
        for k in range(len(c)):
            ...
            # for n in range(200):
                label.append(1)
                data1, data2 = dataGenerate_dep(number[i][0], number[i][1], sample[j], c[k])
                ...
                label.append(0)
                data1_, data2_ = dataGenerate_ind(number[i][0], number[i][1], sample[j])
            ...
            df.to_csv('D:/csv_cosine/combination%d-%d-%d.csv'%(i,j,k), index = False, header = True)
...
for i in range(len(number)):
    for j in range(len(sample)):
        for k in range(len(c)):
            data = pd.read_csv('D:/csv_cosine/combination%d-%d-%d.csv'%(i,j,k))
