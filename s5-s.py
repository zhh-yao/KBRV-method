...
def dataGenerate_dep(num1_exon, num2_exon, S, co):
    ...
    if num1_exon >= num2_exon:
        ...
        isoform2_expression = isoform2_expression + co*(isoform1_expression[:,0:num2_exon])**2
    else:
        isoform1_expression = isoform1_expression + co*(isoform2_expression[:,0:num1_exon])
        isoform2_expression = isoform2_expression + co*(isoform2_expression[:,0:num2_exon])**2
...
for i in range(len(number)):
    for j in range(len(sample)):
        for k in range(len(c)):
            ...
            df.to_csv('D:/csv_square/combination%d-%d-%d.csv'%(i,j,k), index = False, header = True)
...
for i in range(len(number)):
    for j in range(len(sample)):
        for k in range(len(c)):
            data = pd.read_csv('D:/csv_square/combination%d-%d-%d.csv'%(i,j,k))
